...
def clear_of_menu(window):
    ...
    menu.add_command(label="Настройки", command=lambda: configs.configs(window, _event))
...
class Object_Button():
    ...
    def k(self):
        global num_of_click, number_of_squares
        info = f.open_file()
        try:
        ...
        number_of_squares = int(list_with_info[1])
...
class Game:
    ...
    def start(self):
        ...
        window.title("Game")
        window.iconbitmap(r"E:\New folder\programs\git\Code\game.ico")
        window.config(bg=info[0])
